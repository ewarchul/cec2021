...
ew_ipop_cma_esr_ppmf = function(
  ...
  sigma.log = numeric(ceiling(budget / (4 * n)))
  bestPop.log = numeric(ceiling(budget / (4 * n)))
  ...
  for (run in 0:max.restarts) {
    ...
    # restart triggered
      ...
      bestPop.log[iter] = fitn.ordered[1]
      ...
      if ((iter %% 10 == 0) && (abs(best.fitness - fitn.ordered[1]) < 0.5)) {
        if (p_target <=  0.5) {
  ...
  log$bestPop = bestPop.log[1:iter]
