...
#'
#' @description 
#' Implementation of https://hal.inria.fr/hal-00801414/ with
#' {cmaesr} package https://github.com/jakobbossek/cmaesr.
#' Check for comparision MSR implementation in Python:
#' https://github.com/CMA-ES/pycma/blob/master/cma/sigma_adaptation.py
#' look for: class CMAAdaptSigmaMedianImprovement(CMAAdaptSigmaBase)

rb_ipop_cma_esr_msr = function(
  par,
  fn,
  ...,
  lower = -100,
  ...
  #' sign-rule
  c_sigma = getCMAESParameter(control, "c_sigma", 0.3)

  #' damping factor
  d_sigma = getCMAESParameter(control, "d_sigma", 2 * (n - 1) / n)
  ...
  for (run in 0:max.restarts) {
    ...
    #' MSR averaged statistic
    s = 0

    #' MSR helper viarable to hold ordered
    #' fitness values from previous iteration
    previous_pop = matrix(0, nrow = lambda, ncol = 2)
    ...
    # restart triggered
      ...
      #' Update step-size sigma with MSR (Median Success Rule)
      previous_pop[, base::`%%`(iter, 2) + 1] = fitn.ordered
      #' mindex stands for j(lambda, n) in the paper
      fitness_to_compare = previous_pop[, base::`%%`(iter - 1, 2) + 1][mindex]
      K_succ = length(which(fitn.ordered < fitness_to_compare))
