...
rb_ipop_cma_esr_ppmf_bounce = function(
  par,
  fn,
  ...,
  lower = -100, 
  ...
  last_its_type = getCMAESParameter(control, "last_its_type", "ave")
  ...
  for (run in 0:max.restarts) {
    ...
    # restart triggered
      ...
      arx.repaired = apply(arx, 2, bounceBackBoundary)
      ...
      fitn.ordered.idx = order(fitn.repaired, decreasing = FALSE)
      fitn.ordered = fitn.repaired[fitn.ordered.idx]

      bestVal.log = rbind(bestVal.log, min(suppressWarnings(min(bestVal.log)), min(fitn.ordered)))

      # update best solution so far
      min.valid.idx = which.min(fitn.repaired)

      if (fitn.repaired[min.valid.idx] < best.fitness) {
        best.fitness = fitn.repaired[min.valid.idx]
        best.param = arx[, min.valid.idx]
      }
...
bounceBackBoundary <- function(x, lb = -100, ub = 100) {
    n = length(x)
    lower = rep(lb, n)
    upper = rep(ub, n)
    x = cbind(x)
